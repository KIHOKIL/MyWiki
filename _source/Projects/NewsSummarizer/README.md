...
## ✨ 핵심 기능
- **🤖 Gemini AI 분석**: 단순 요약을 넘어 여러 기사를 맥락 있게 분석해 줍니다.
- **✉️ 아침 메일 브리핑**: 매일 지정된 시간에 이메일로 리포트를 쏴줍니다.
- **💡 스마트 트렌드 추천**: 메일 하단에 내일 추가하면 좋을 새로운 트렌드 키워드를 추천해 줍니다.
- **📱 스마트폰 원격 업데이트 (IssueOps)**: 추천받은 주제를 추가하고 싶다면? PC를 켤 필요 없이 스마트폰 GitHub 앱에서 Issue만 남기면 AI가 알아서 설정을 업데이트해 줍니다.
- **📝 Obsidian 완벽 호환**: 요약본은 매일 Markdown으로 변환되어 내 저장소에 차곡차곡 쌓입니다.
