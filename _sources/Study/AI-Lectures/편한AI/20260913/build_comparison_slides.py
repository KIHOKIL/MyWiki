...
def build_slides():
    ...
    # 마크다운 파일 저장 (한글 인코딩 방지를 위해 utf-8)
    md_path = os.path.join(slides_dir, "comparison_slides.md")
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(slide_content)
        
    print(f"✅ Slide markdown created at {md_path}")
    
    # 여기서 Marp CLI를 호출하여 HTML을 생성할 수 있습니다.
    # 현재는 목업용 HTML 래퍼를 생성합니다.
    html_content = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>비교 분석 슬라이드</title>
    <!-- Simple reveal.js or marp fallback style -->
    <style>
        body {{ margin: 0; padding: 20px; font-family: sans-serif; background: #0f172a; color: white; }}
        .slide {{ border: 1px solid #334155; border-radius: 10px; padding: 40px; margin-bottom: 20px; background: #1e293b; min-height: 400px; }}
        h1 {{ color: #60a5fa; }}
        h2 {{ color: #c084fc; border-bottom: 1px solid #334155; padding-bottom: 10px; }}
        strong {{ color: #3b82f6; }}
    </style>
</head>
<body>
    <h1>Obsidian LLM Wiki 3종 비교 슬라이드 뷰어</h1>
    <p>아래는 슬라이드 마크다운의 내용을 웹용으로 분할 렌더링한 형태입니다.</p>
"""
    
    slides = slide_content.split("---")
    for i, slide in enumerate(slides[1:], 1): # 첫번째 meta 생략
        if not slide.strip(): continue
        # 아주 간단한 마크다운 파싱
        html_slide = slide.replace("\n\n", "<br>").replace("# ", "<h1>").replace("## ", "<h2>").replace("* ", "<li>")
        html_content += f'<div class="slide">{html_slide}</div>\n'
        
    html_content += "</body></html>"
    
    html_path = os.path.join(slides_dir, "index.html")
    with open(html_path, "w", encoding="utf-8") as f:
        f.write(html_content)
        
    print(f"✅ Slide HTML viewer created at {html_path}")
    print("🎉 Done!")
